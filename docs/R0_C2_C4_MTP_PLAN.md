...
Keep the production non-KVMEM path fixed: COOP=1, PIECEWISE graphs,
text-only, disk n-gram, profiler off, prefix caching off, `GPU_MEM_UTIL=0.92`,
`MAX_MODEL_LEN=246000`, `MAX_NUM_SEQS=4`, and
...
- Before requests, record the actual KV pool and require capacity for the
  ...
  the fixed 0.92/246000 envelope. The earlier k3 C1 pool of 279,087 tokens is
