#!/usr/bin/env python3
"""Build the K1-Q2B CPU-backed resident cache plan from frozen K1B."""
...
def build_plan(
    ...
    *,
    ...
    if publication_staging_pages <= 0:
        raise ValueError("publication staging pages must be positive")
    ...
    if q0 % page_tokens:
        raise ValueError(
            "Q2B requires query boundary aligned to a KV page for clean remap"
        )
    ...
    return {
        ...
        "publication_staging_pages": min(
            publication_staging_pages, resident_page_count
        ),
        "transfer_tensor_page_count": (
            physical_page_count
            + min(publication_staging_pages, resident_page_count)
        ),
        "query_span": [q0, q1],
        ...
        "note": (
            "Q2B keeps the scheduler/full cache intact as a same-forward "
            "reference only. Historical resident pages are gathered through a "
            "small GPU staging window, published via vLLM generic CPU offload, "
            "then staged back into the independent 16-token resident cache. "
            "page_tokens is independent of the hybrid scheduler full-block size."
        ),
    }
...
def main() -> int:
    ...
    ap.add_argument("--publication-staging-pages", type=int, default=128)
    ...
    result = build_plan(
        ...
        publication_staging_pages=args.publication_staging_pages,
    )
