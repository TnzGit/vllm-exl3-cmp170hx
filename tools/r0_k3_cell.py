...
def main() -> int:
    ...
    for _ in range(args.repeats):
        ...
        # Counter identity around sequence boundaries:
        #   derived = verification_passes + accepted_draft_tokens.
        # The API usage count can be one token larger because the initial target
        # token precedes the first speculative pass. Conversely, a max_tokens
        # truncation can make the final speculative pass account for up to k
        # more derived tokens than the API emits. Therefore the valid range is
        #   -1 <= derived - usage <= k
        # where k is measured from draft_tokens/pass for this same interval.
        spec_k = (
