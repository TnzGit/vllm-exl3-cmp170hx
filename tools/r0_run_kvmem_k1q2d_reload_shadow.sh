#!/usr/bin/env bash
# K1-Q2D full-source correctness oracle for bounded CPU-backed QSA reload.
set -euo pipefail
...
OUT="${1:-$R/results/kvmem-k1q2d-reload-shadow}"
EXPECTED_SHA="${EXPECTED_SHA:?set EXPECTED_SHA to the exact Q2D head}"
PORT="${PORT:-8002}"
...
Q2C_PLAN="$OUT/q2c_runtime_plan.json"
PLAN="$OUT/q2d_reload_plan.json"
QSA_BACKUP="$OUT/qsa.base.py"
STATS="$OUT/q2d_reload_stats.jsonl"
RESPONSE="$OUT/q2d_reload_response.json"
SUMMARY="$OUT/q2d_reload_summary.json"
LOG="$OUT/logs/serve_q2d_reload.log"
LAUNCH_PID=""
PATCHED=0
...
restore_qsa() {
  if [[ "$PATCHED" == "1" && -f "$QSA_BACKUP" ]]; then
    cp "$QSA_BACKUP" "$QSA"
    rm -f "$QSA.kvmem_qsa_q2d_reload_shadow.orig"
    PATCHED=0
  fi
}
...
echo "=== CPU gates ==="
bash -n "$REPO/tools/r0_run_kvmem_k1q2d_reload_shadow.sh"
"$V/bin/python" -m py_compile \
  "$REPO/src/vllm_exl3/kvmem_q2d_reload_shadow.py" \
  "$REPO/src/vllm_exl3/kvmem_q2d_plan.py" \
  "$REPO/src/vllm_exl3/kvmem_vllm_offload.py" \
  "$REPO/tools/kvmem_qsa_make_q2d_reload_plan.py" \
  "$REPO/tools/kvmem_q2d_reload_summarize.py" \
  "$REPO/tools/patch_vllm_qwen4_exp/patch_vllm_qsa_q2d_reload_shadow.py"
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  "$V/bin/python" "$REPO/tools/patch_vllm_qwen4_exp/patch_vllm_qsa_q2d_reload_shadow.py" \
  "$VLLM_ROOT" --check-only
cmp -s "$QSA_BACKUP" "$QSA"
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  "$V/bin/python" -m pytest -q \
  "$REPO/tests/test_kvmem_q2d_reload_shadow.py" \
  "$REPO/tests/test_qsa_q2d_reload_shadow_patch.py" \
  "$REPO/tests/test_kvmem_q2d_reload_summary.py" \
...
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  ...
  > "$OUT/q2c_runtime_plan.stdout.json"
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  "$V/bin/python" "$REPO/tools/kvmem_qsa_make_q2d_reload_plan.py" \
  --q2c-plan "$Q2C_PLAN" --out "$PLAN" \
  | tee "$OUT/q2d_reload_plan.stdout.json"

XID0=$(xid_now); XID0=${XID0:-0}
echo "xid_before=$XID0"

echo "=== apply Q2D reload shadow patch ==="
PATCHED=1
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  "$V/bin/python" "$REPO/tools/patch_vllm_qwen4_exp/patch_vllm_qsa_q2d_reload_shadow.py" \
  "$VLLM_ROOT"
grep -Fq "# KVMEM_QSA_Q2D_RELOAD_SHADOW_V1" "$QSA"

echo "=== boot and run 160K Q2D reload oracle ==="
PYTHONPATH="$REPO/src${PYTHONPATH:+:$PYTHONPATH}" \
VLLM_QWEN_KVMEM_Q2D_PLAN="$PLAN" \
VLLM_QWEN_KVMEM_Q2D_STATS_PATH="$STATS" \
...
"$V/bin/python" "$REPO/tools/kvmem_qsa_visibility_probe.py" \
  ...
  --out "$RESPONSE" | tee "$OUT/q2d_reload_response.stdout.json"
guard_idle
stop_engine
test -s "$STATS"
...
set +e
PYTHONPATH="$REPO/src:$REPO${PYTHONPATH:+:$PYTHONPATH}" \
  "$V/bin/python" "$REPO/tools/kvmem_q2d_reload_summarize.py" \
  --response "$RESPONSE" --stats "$STATS" --plan "$PLAN" --out "$SUMMARY" \
  | tee "$OUT/q2d_reload_summary.stdout.json"
SUMMARY_RC=$?
set -e
